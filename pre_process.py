...
def balanced_split(f_test, final_genomes, taxid_to_tnum):
    """Create a train-test split that is phylogenetically balanced at the
    phylum level.
    ...
    for taxid in final_genomes:
        ...
        if "k__Bacteria" not in final_genomes[taxid]:
            continue
        try:
...
def genomes2include(path):
    """Figure out which genomes I want to include in my dataset based on those
    in selected_kegg.txt.
...
def load_kos(tla_to_tnum, tnum_to_tla, tla_to_mod_to_kos, path):
    ...
    master_file = open(config.DATA_ROOT + config.DL_ANNOTATIONS_PATH).readlines()
    ...
    for i in master_file:
        file = open(config.DATA_ROOT + config.ANNOTATIONS_PATH + i).readlines()
        file = list(map(str.strip, file))
        ...
        try:
            ...
            continue  # not all will be present in keepers (e.g. "Pea")
...
def make_tensor(
    tla_to_mod_to_kos, tnum_to_kos, n_kos_tot, tla_to_tnum, all_kos, save=False
):
...
def clean_kos(mod_sets):
    """Creates dict mapping modules the most common version (set of encoding
    genes) therefore.
    ...
    for mod in mod_sets:
        ...
        for ko_str in mod_sets[mod]:
            if (
                mod_sets[mod][ko_str] > max_count
            ):  # if there is a tie, the first one is kept
...
def thin2():
    ...
    master_file = open(
        "/Users/natasha/Desktop/kegg_mar2021/downloaded_infoFiles.txt"
    ).readlines()
    master_file = map(str.strip, master_file)

    tax_path = open(
        "/Users/natasha/Desktop/mcgill_postdoc/ncbi_genomes/ncbi_lineages_2020-05-04.csv"
    ).readlines()
    ...
    for s in tax_path:
        ...
        if len(classS) < 1:
            ...
            tax_dict[taxid] = (
                "k__" + domain + ";p__" + phylum + ";c__" + classS + ";o__" + order
            )
        elif len(genus) < 1:
            tax_dict[taxid] = (
                "k__"
    ...
    for s in spp_to_genome:
        if ";s__" in s:
            ...
            if num_per_genus[genus] <= num_thresh + 1:
                ...
                if (
                    "Babela massiliensis" in s
                ):  # Phylum for this spp is undefined, should be TM6
    ...
    for (
        ...
        spp_to_genome
    ):  # s is something like 'p__Proteobacteria;c__Gammaproteobacteria;o__Oceanospirillales'

        phylum = s.split(";")[1]

        if (
            ";s__" not in s and len(phylum) > 3
        ):  # get rid of anything not classified at phylum level p__
    ...
    for phylum in unk_per_phylum:
        ...
        if len(unk_per_phylum[phylum]) > max_per_phy:
            selected_genomes = random.sample(
                unk_per_phylum[phylum], k=max_per_phy
            )  # random.sample does not use replacement
        else:
...
def thin3():
    ...
    for i in master_file:
        file = open(
            "/Users/natasha/Desktop/mcgill_postdoc/ncbi_genomes/kegg_dataset/kegg_dl/"
            + i
        ).readlines()
        file = map(str.strip, file)
...
def load_data(DATA_DIR):
    ...
    Arguments:
            DATA_DIR (str) -- path to KEGG and other data
    ...
    """
    tla_to_mod_to_kos, mod_sets = load_mods(DATA_DIR)
    tla_to_tnum, tnum_to_tla, keepers = genomes2include(DATA_DIR)
    tnum_to_kos, n_kos_tot, all_kos = load_kos(
        tla_to_tnum, tnum_to_tla, tla_to_mod_to_kos, DATA_DIR
    )
    mod_to_kos = create_mod_to_kos(tla_to_mod_to_kos)
    mod_to_ko_clean = clean_kos(mod_sets)

    all_kos = torch.load(DATA_DIR + "all_kos_2020-09-29.pt")
    train_data = torch.load(DATA_DIR + "kegg_train_data_2020-09-29.pt")
    test_data = torch.load(DATA_DIR + "kegg_test_data_2020-09-29.pt")
    train_genomes = torch.load(DATA_DIR + "kegg_train_genomes_list_2020-09-29.pt")
    test_genomes = torch.load(DATA_DIR + "kegg_test_genomes_list_2020-09-29.pt")
...
def tax_dicts(c_train_genomes, train_input_mods, c_test_genomes, test_input_mods):
    """Create dict that maps each tla to its taxonomic ID -- 'tla': ['domain',
    'phylum', ..., 'species']
    ...
    master_file = open(config.DATA_ROOT + config.KEGG_DL_PATH).readlines()
    master_file = list(map(str.strip, master_file))

    taxid_to_tla = {}

    for i in master_file:
        file = open(config.DATA_ROOT + config.INFO_PATH + i).readlines()
        file = map(str.strip, file)

        threeLA = i.split("_")[
            0
        ]  # short for KEGG's 'three letter abbreviation', e.g.: rav for T03876
    ...
    def final_sets(
        c_genomes, taxid_to_tla
    ):  # c_genomes is either c_train_genomes or c_test_genomes
        tax_path = open(config.DATA_ROOT + config.NCBI_LINEAGES_PATH).readlines()
        tax_path = map(str.strip, tax_path)
