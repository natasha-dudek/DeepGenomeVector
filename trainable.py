...
def train_single_vae(nn_layers, weight_decay, lr, batch_size, kfolds, num_epochs, replacement_threshold):
    """
    Train autoencoder, save model and y_probas 
    ...
    model = models.VariationalAutoEncoder(num_features2, nn_layers)
    model = model.to(device)
    model.train()
