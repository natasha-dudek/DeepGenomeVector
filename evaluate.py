...
from sklearn.metrics import (accuracy_score, auc, confusion_matrix,
                             hamming_loss, pairwise_distances, roc_auc_score,
                             roc_curve)
...
def eval_binarize(pred, replacement_threshold):
    """Convert tensor of probabilites to binary values, using
    ...
    return torch.LongTensor(
        Binarizer(threshold=replacement_threshold).fit_transform(pred)
    )
...
def confusion(uncorrupted, binary_pred):
    ...
    print(
        "The percentage of TNs, FPs, FNs, and TPs, respectively, is:",
        p_tns,
        "(%)",
        p_fps,
        "(%)",
...
def pixel_diagram(
    corrupted, uncorrupted, idx, model, f1s, tns, fps, fns, tps, binary_pred
):
    """Plot a pixel diagram (heatmap) visualizing the number of TNs, FPs, FNs,
    TPs.
    ...
    print(
        "Corrupted -- Genes on:",
        str(int(sum(sum(corrupted)))),
        "Genes off:",
        str(int(n_features - sum(sum(corrupted)))),
    )
    ...
    print(
        "Uncorrupted -- Genes on:",
        str(int(sum(sum(uncorrupted)))),
        "Genes off:",
        str(int(n_features - sum(sum(uncorrupted)))),
    )
...
def baseline1(
    ...
    def gene_dist(tla_to_mod_to_kos, tnum_to_kos, tla_to_tnum, c_train_genomes):
        ...
        for org in tla_to_mod_to_kos:
            ...
            if (
                org in c_train_genomes
                and len(tla_to_mod_to_kos[org]) >= 10
                and len(tnum_to_kos[tnum]) >= 400
            ):  # make sure org is in training set, not test
    ...
    gene_counts = gene_dist(
        tla_to_mod_to_kos, tnum_to_kos, tla_to_tnum, c_train_genomes
    )
...
def baseline4(
    corrupted_train, corrupted_test, tla_to_tnum, tnum_to_kos, c_train_genomes
):
    """
    ...
    start = c_train_genomes.index(
        smallest_tla
    )  # hed = Hoaglandella endobia, Gammaproteobacteria
    smallest_uncorrupted = corrupted_train[start, n_features:]

    # Create baseline for test set
    baseline4 = torch.Tensor(
        np.tile(smallest_uncorrupted, (corrupted_test.shape[0], 1))
    )
...
def generated_inputs_to_binary(generated_inputs, all_kos):
    """Take list of KOs used as input to VAE to generate new genome vector,
    convert to binary matrix matching order of all_kos.
...
def compare_in_n_out(binary_pred, generated_inputs_binary):
    """Plot histogram showing how often genes in the VAE input are also in the
    ...
    print(
        ...
        total,
        "instances or",
        str(round(count_hund / total * 100, 2)) + "% of cases",
    )
    print(
        "There are",
        count_ninety,
        "instance of inputs being >=90% present in output ("
...
def best_med_worst(f1s, c_test_genomes, tla_to_tnum):
    """Get the best, median, and worst reconstructions from the test set, as
...
def f1s_per_phylum(train_tax_dict, test_tax_dict, c_test_genomes, f1s):
    """For training set, creates a dict counting the number of genome vectors
    (values) per phylum (keys). For test set, creates a dict of phyla (keys) +
    list of F1 scores of all genome vectors in that phylum (values).
...
def ngenesUncorrupted_vs_f1(uncorrupted_test, f1s, ax=None):
    """Plots scatterplot showing # genes in uncorrupted training genomes vs F1
    score of genes.
    ...
    """
    n_genes_uncorrupted = (
        torch.sum(uncorrupted_test, 1).numpy().tolist()
    )  # get sum of each row
    ...
    xseq = np.linspace(
        min(n_genes_uncorrupted) - 50, max(n_genes_uncorrupted) + 50, num=100
    )
...
def ngenesCorrupted_vs_f1(corrupted_test, f1s):
    """Plots scatterplot showing # genes in corrupted training genomes vs F1
    score of genes.
    ...
    """
    n_genes_corrupted = (
        torch.sum(corrupted_test, 1).numpy().tolist()
    )  # get sum of each row
...
def plot_train_count_hist(train_input_mods):
    """Plots histogram showing the # of times each mod is used in a corrupted
    genome during training.
...
def test_f1s(uncorrupted, binary_pred):
    """Calculate F1 scores for all genomes in the test set and plot a
    histogram.
...
def quantEval(
    ...
    """Plots two learning curves (optimization, performance), an ROC curve, and
    histogram of F1 scores -- i.e. a figure for the manuscript.
    ...
    axs[0][0].set_xlim(
        -50, x_losses[-1] + 5
    )  # tweaking left side because otherwise the first data point is hard to see on the plot
...
def genus_boxplot_stats(groups):
    """Perform anova and tukey test to accompany analysis in genus_boxplot
    function.
...
def plot_mod_count_vs_f1(test_input_mods, f1s):
    """Create scatterplot of number of mod occurences in the corrupted training
    dataset vs F1 score on the test set.
...
def plot_metab_pathway_f1_horizontal(
    process_to_mod, mod_to_kos_clean, all_kos, ko_f1s, figsize
):
    """Generate box plot showing F1 scores of genes within processes or
    subprocesses.
    ...
    list_medians, list_f1s, list_procs = zip(
        *sorted(zip(list_medians, list_f1s, list_procs), reverse=False)
    )
    ...
    plt.yticks(
        [i + 1 for i in range(len(list_procs))],
        [proc for proc in list_procs],
        rotation=0,
    )
    plt.xlabel("F1 score")
    ax.set_xlim(0, 1)
...
def export_selected_generated(OUT_DIR, gen_kos, gen_idx):
    """Export a particular generated genome from our set of n used for the
    paper analysis.
...
def generate_genomes(n_gen, all_kos, mod_to_kos, n_mods, model):
    ...
    generated_inputs = (
        {}
    )  # track which modules were used as inputs for each generated genome
...
def estimate_hamming_novelty(test_data, generated, train_data):
    """Calculate minimum hamming distance between a) generated genomes to
    training genomes, b) test set genomes to training genomes. Use the Mann-
    Whitney U test to evaluate whether the distributions are different.
    ...
    print(
        "test set: median, MAD, min, and max",
        np.median(minh_test),
        stats.median_absolute_deviation(minh_test),
        min(minh_test),
        max(minh_test),
    )
    print(
        "generated: median, MAD, min, and max",
        np.median(minh_gen),
        stats.median_absolute_deviation(minh_gen),
        min(minh_gen),
        max(minh_gen),
    )
...
def plot_hamming_novelty(minh_test, minh_gen):
    """Plot histogram showing minimum hamming distance between a) generated
    genomes to training genomes, b) test set genomes to training genomes. Use
    the Mann-Whitney U test to evaluate whether the distributions are
    different.
...
def pca_gen_vs_real(generated, test_data, idx=None):
    """Plot PCA of Jaccard similarity between genomes, using Hamming distances
    as a metric.
...
def write_out_for_phylip(OUT_DIR, df, tnum_to_tla, test_tax_dict):
    """Converts df of genome vectors into character matrix that can be input to
    Phylip, writes to disk.
...
def get_phyla_colours():
    """Returns pre-defined dict mapping phyla in test set to a unique colour
    (rbg)
...
def colour_real_itol(OUT_DIR, phyla_colours, phylum_dict):
    """Creates iTOL colorstrip file for gene +/- dendrogram, colours = phyla of
    test genomes.
    ...
    label_legend, colour_legend = zip(
        *sorted(zip(label_legend, colour_legend), reverse=False)
    )
...
def colour_generated_itol(OUT_DIR, phylum_dict):
    """Creates iTOL colorstrip file for gene +/- dendrogram, colours = real vs
    generated genome.
...
def bio_insights_fig(test_phyla, subprocess_to_mod, all_kos, ko_f1s, mod_to_kos_clean):
    """Generate two-panel figure; Panel 1 shows the median F1 score for test
    genomes from different phyla, Panel 2 shows the median F1 score of genes
    within different subprocesses.
    ...
    median, mad, phylum_list = zip(
        *sorted(zip(median, mad, phylum_list), reverse=False)
    )
    phylum_num = [i for i in range(len(phylum_list))]
...
def confusion_barplot(
    f1s, c_test_genomes, tns, fps, fns, tps, uncorrupted, corrupted, idx
):
    """Create barplot showing # TNs, TPs, FNs, and FPs for original, corrupted,
    and reconstructed test genomes.
    ...
    indices = [
        i for i in range(len(f1s))
    ]  # original index of each genome in uncorrupted
    ...
    total = sum(data1)
    perc1 = [
        round(tp_best / total * 100, 2),
        round(tn_best / total * 100, 2),
        round(fn_best / total * 100, 2),
        round(fp_best / total * 100, 2),
    ]
...
def get_mod_names():
    """Get names of all modules (e.g.: 'M00001': 'Glycolysis (Embden-Meyerhof
    pathway)')
    ...
    for s in file:
        if s[0] == "D":
            mod_names[s.split()[1]] = (
                " ".join(s.split()[2:]).split("[")[0].split(",")[0]
            )
...
def compare_inputs(
    test_input_mods, idx, tla_to_mod_to_kos, train_genomes, tla_to_tnum, mod_names
):
    """For a given generated genome vector, figure out how many training set
    genome vectors encoded all 10 of the modules used as input to generate our
    selected genome vector. Create a barplot.
...
def compare_venn(name1, name2, name3, df):
    """Make Venn diagram showing overlap in gene IDs between three genome
    vectors.
    ...
    fig = venn3(
        [set(shared1), set(shared2), set(shared3)], set_labels=(name1, name2, name3)
    )
...
def get_ten_closest(
    ...
    """For a given reconstructed genome vector, get the ___ and ___ of the most
    similar real training genome vectors (similar is defined by the Hamming
    distance)
    ...
    idx_v2 = test_genomes.index(
        tnum_x
    )  # indexing is different for binary_pred and test_genomes
    ...
    hammings, train_genomes_sorted, idxs = zip(
        *sorted(zip(hammings, train_genomes, idxs), reverse=False)
    )
    hamm_10 = hammings[:10]
    ...
    labels = [train_tax_dict[tnum_to_tla[tnum]][6] for tnum in closest_genomes] + [
        test_tax_dict[tnum_to_tla[tnum_x]][6],
        "Generated",
    ]

    ten_df = pd.DataFrame(
        np.vstack((unc_train_data[idx_10, :], orig, generated)), labels
    )
...
def nmods_vs_f1(
    ...
    """Plots scatterplot showing correlation between F1 score of reconstruction
    using a given module as input and how many training set genomes encode that
    module.
...
def genus_boxplot(
    c_test_genomes, tla_to_tnum, train_genomes, tnum_to_tax, f1s, ax=None
):
    """Barplot showing the # of same-genus genome vectors in the training set
    vs test set F1 scores.
    ...
    tax_groups = {
        ...
        "genus": [],
        "species": [],
    }  # taxonomic groups in training set
    ...
    for genus in genus_count:
        if genus_count[genus] > 6:
            continue  # skip '' (unknown genus)
        if genus_count[genus] == 0:
            group_0.append(np.median(genus_f1[genus]))
        if genus_count[genus] == 1:
            group_1.append(np.median(genus_f1[genus]))
        if genus_count[genus] == 2:
            group_2.append(np.median(genus_f1[genus]))
        if genus_count[genus] == 3:
            group_3.append(np.median(genus_f1[genus]))
        if genus_count[genus] == 4:
            group_4.append(np.median(genus_f1[genus]))
        if genus_count[genus] == 5:
            group_5.append(np.median(genus_f1[genus]))
...
def f1_per_ko(uncorrupted_test, binary_pred, train_data):
    ...
    for i in range(uncorrupted_test.shape[1]):  # for every column
        f1 = sk.metrics.f1_score(
            uncorrupted_test[:, i], binary_pred[:, i], zero_division=0
        )
        ko_f1s.append(f1)
    ...
    print(
        "There are",
        zeros,
        "KOs with F1=0 out of",
        len(ko_f1s),
        "KOs. That is",
        zeros / len(ko_f1s) * 100,
        "%",
    )
...
def geneCount_vs_geneF1(corrupted_train, num_features, ko_f1s, ax=None):
    """Create scatter plot of gene count in the uncorrupted training set vs per
    gene test F1 score.
...
def complete_mods(generated, all_kos, mod_to_ko_clean):
    """Calculate the number of complete modules (all req'd KOs are present) in
    a set of genomes.
...
def mod_freqs(mod_to_ko_clean, test_data, generated, real_mods, gen_mods):
    """Calculate the frequency of modules in real test genomes and generated
    genomes.
    ...
    real_mod_freq, gen_mod_freq = zip(
        *sorted(zip(real_mod_freq, gen_mod_freq), reverse=True)
    )
...
def dist_genes_mods(generated, all_kos, mod_to_ko_clean, test_data):
    """Generates three panel figure. Panel 1 is a barplot of # of genes vs
    genome count, Panel #2 is a barplot of # of complete modules vs genome
    count, and Panel #3 is a barplot of module vs fractino of genomes encoding
    that module.
    ...
    real_mod_freq, gen_mod_freq = mod_freqs(
        mod_to_ko_clean, test_data, generated, real_mods, gen_mods
    )
...
def id_incomplete_mods(generated_inputs, gen_idx, mod_to_ko_clean, gen_kos):
    """Identify incomplete modules in a generated genome vector, learn more
    about how they are incomplete.
