...
class VariationalAutoEncoder(nn.Module):
    """A VariationalAutoEncoder object contains a VAE network comprised of an
    encoder and decoder.

    The encoder compresses and reparameterizes input to a constrained
    multivariate latent distribution. The decoder strives to map a
    sample from this distribution back to the original version of the
    input.
    """
    ...
    def encode(self, x):
        """Map each input data point to mean and logvariance vectors that
        define the multivariate normal distribution around that data point.
    ...
    def reparameterize(self, mu, logvar):
        """Return a sample from the multivariate normal distribution around
        each data point.
