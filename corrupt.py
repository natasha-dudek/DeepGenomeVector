...
def heart_of_corruption(org_to_mod_to_kos, org, n_kos_tot, all_kos, n_mods):
	"""For each genome vector, keep the KO's in n_mods modules. All other bits
...
def corrupt(
	...
	"""Perform corruptions on a set of genome vector. This means keep the KO's
	in n_mods modules for each genome vector. All other bits will be converted
	to zeros.
	...
	for i, tnum in enumerate(train_genomes):
		...
		while n_corrupted < n_corrupt:
			...
			corrupted, in_mods = heart_of_corruption(
				org_to_mod_to_kos, org, n_kos_tot, all_kos, n_mods
			)
...
def new_corrupt(
	...
	Arguments:
			...
			date_to_save (str) -- unique identifier for saved filenames
			
	Returns:
			corrupted_train (tensor) -- uncorrupted and corrupted training data. Rows are genomes, columns are genes. 1 denotes gene encoded by genome, 0 denotes absence.
	...
	corrupted_train, c_train_genomes, train_input_mods = corrupt(
		train_data,
		train_genomes,
		n_corrupt,
		tnum_to_tla,
	...
	corrupted_test, c_test_genomes, test_input_mods = corrupt(
		test_data,
		test_genomes,
		n_corrupt,
		tnum_to_tla,
